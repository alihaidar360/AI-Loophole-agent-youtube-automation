...
Provider strategy: gpt-oss-120b (OpenAI's open-weight model — genuinely
stronger reasoning than what Gemini 2.5 Flash offered) via TWO different
free, stable inference providers (Groq, then Cerebras) for redundancy,
with a proven Llama model as final safety net. No Google Gemini
dependency — its model names/SDK have changed multiple times this year,
which made it an unreliable dependency for unattended automation.

Long-form & Shorts: Groq gpt-oss-120b -> Cerebras gpt-oss-120b -> Groq llama-3.3-70b-versatile
"""
...
("really good", "very fast"). Target spoken length: 10-13 minutes
(~1500-2000 words).

If the user message includes "what_has_worked_recently", treat it as real
performance data from this channel's past videos and lean into it where
honest. If it includes "cross_promotion", reference that other video
naturally, once, near the end.
...
one genuine limitation, not just hype — this is what makes it feel real
instead of like an ad.
...
def _extract_json(text: str) -> dict:
    ...
    then parses. Raises if the result still isn't valid JSON, which the
    fallback chain treats as a provider failure and moves to the next one."""
...
def _critic_pass(script: dict, research: dict) -> dict:
    """Second LLM pass that critiques the draft against a concrete
    checklist and returns a revised version. Never blocks the pipeline —
    if this call fails, the original (already good) draft is used."""
    try:
