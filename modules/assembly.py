...
IMPORTANT (learned the hard way): Remotion's renderer refuses file://
URLs for local assets ("Not allowed to load local resource"). The
supported approach is: pass --public-dir pointing at assets/, then give
the JS side paths RELATIVE to that folder — the React components wrap
them with Remotion's staticFile() helper.
"""
...
def _rel_to_assets(p: str) -> str:
    """Converts a local path into one relative to assets/ — what
    staticFile() on the JS side expects."""
...
def assemble_video(video_type: str, audio_path: str, visual_paths: list,
    ...
    # Defensive checks: a path recorded in job_state.json doesn't guarantee
    # the binary still exists on disk (GitHub Actions cache can be stale
    # across a job resumed over multiple days/runs). Skip missing
    # music/SFX rather than crash the whole video over it.
